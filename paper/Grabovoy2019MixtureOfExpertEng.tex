...
\title{\bf Priori distribution choices for a mixture of experts \thanks{This research was supported by RFBR (project ???) and NTI (project ???).}}
\date{}
\author{}
\maketitle
...
A mixture of expert is a set of experts and gate function which weighs these experts.
...
In this article, we analysed different a priori distributions for each expert.
We proposed a method that consider the relationship between the a priori distributions of different experts.
%Анализируется случай, когда выбрано информативное и неинформативное априорные распределения параметров каждого эксперта.
To solve the obtained optimization problem, we used the EM algorithm.
%Рассматривается задача поиска окружностей на изображении.
The problem of finding circles in the image is considered as the task of a mixture of experts.%Каждой окружности на изображении соответствует свой эксперт. 
Each circle in the image is a one expert.
%Требуется найти на изображении синтетически сгенерированные окружности с разным уровнем шума.
Our method was tested on the real and synthetic data. A real data is a human eyes from rhe iris detection problem.
...
\end{comment}
\section{Problem statement of finding the circle parameters in the image}
...
where~$1$ is a black point --- an image and $0$ is a white point --- image background.
%По изображению $\textbf{M}$ строится выборка $\textbf{C}$, элементами которой являются координаты~$x_i, y_i$ черных точек на картинке:
The set~$\textbf{C}$ constructed by mapping the image $\textbf{M}$ to the set of coordinates~$x_i, y_i$ of black points in the image:
...
where~$N$ is a number of black points in the image~$\textbf{M}$.

%Обозначим $x_0, y_0$ --- центр окружности, которую требуется найти на бинарном изображении $\textbf{M}$, а $r$ ее радиус.
Let $x_0, y_0$ is a coordinates of center and $r$ is a radius of circle in the image $\textbf{M}$.
Circle parameters $x_0, y_0, r$ need to be found.
...
The points $\left(x_i, y_i\right)\in\textbf{C}$ is geometric locus of circle points. This locus approximated by using circle equation:
\[
\label{eq:st:cr:3}
\begin{aligned}
...
The equation~\eqref{eq:st:cr:4} is a linear regression problem with following data:
\[
\label{eq:st:cr:5}
\begin{aligned}
...
where parameters~$\textbf{w} = \bigr[w_1, w_2, w_3\bigr]^{\mathsf{T}}$ reconstruct circle parameters~$x_0, y_0, r$:
\[
\label{eq:st:cr:6}
\begin{aligned}
...
The solution of problem~\eqref{eq:st:cr:5} reconstruct circle parameters only when number of circles in image is one.
%В случае, когда на изображении несколько окружностей, предлагается использовать мультимодель.
If the image has several circles, then it is proposed to use a multimodel.
%В ее состав входят линейные модели.
The multimodel consists of linear models.
...
As a multimodel, we consider a mixture of experts model.
...
Let generalize one circle approximation problem in the case when there are several circles.
%Задана выборка из уравнения~\eqref{eq:st:cr:5} :
Consider the data from the equation~\eqref{eq:st:cr:5} without assumption that circle is one:
\[
\label{eq:st:1}
\begin{aligned}
...
where~$N$~--- the number of datum and~$n$~--- is a number of features.
\begin{definition}
\label{def:1}
...
A mixture of experts is a multimodel in which the weight of each local model depends on the object:
\[
\label{eq:st:2}
\begin{aligned}
...
where~$\hat{\mathbf{f}}$~--- multimodel, $\mathbf{f}_k$~--- local models, $\pi_k$~--- gate function, $\mathbf{w}_k$~--- parameters of local model and $\mathbf{V}$~--- parameters of gate function.
\end{definition}
