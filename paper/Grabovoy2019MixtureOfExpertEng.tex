...
The paper proposes to use a probabilistic approach to describe a mixture of experts. Let~$\textbf{y}$ be a random variable with density function~$p\left(\textbf{y}|\textbf{X}\right)$. Let density~$p\left(\textbf{y}|\textbf{X}, \hat{\textbf{f}}\right) $ approximate truth density~$p\left(\textbf{y}|\textbf{X}\right)$:
\[
\label{eq:st:new:1}
\begin{aligned}
p\left(\textbf{y}|\textbf{X}, \hat{\textbf{f}}\right) = \prod_{i=1}^{N}\left(\sum_{k=1}^{K}\pi_kp_{k}\left(y_{i}|\textbf{f}_{k}\left(\mathbf{x}_{i}\right)\right)\right),
\end{aligned}
\]
where~$\hat{\textbf{f}}$ is the mixture of experts model and~$\textbf{f}_k, \bm{\pi}$ are defined by~\eqref{eq:st:3}.

%Combining~\eqref{eq:st:2} and~\eqref{eq:st:3}, obtain the likelihood solution:
Now if we suppose that~$\textbf{w}_k$ is the random variable with density function~$p^{k}\bigr(\mathbf{w}_k\bigr)$, we get full likelihood:
\[
\label{eq:st:4}
\begin{aligned}
...
\section{Probabilistic statement}
To build a mixture of experts, set the following probabilistic statement for the dataset~\eqref{eq:st:1}:

\begin{enumerate}
	\item[1)] the likelihood $p_{k}\left(y_{i}|\mathbf{w}_{k}, \mathbf{x}_{i}\right) = \mathcal{N}\left(y_{i}|\mathbf{w}_{k}^{\mathsf{T}}\mathbf{x}_{i}, \beta^{-1}\right).$ Parameter~$\beta$ is a level of noise,
	\item[2)] the prior distribution for the parameters $p^{k}\left(\mathbf{w}_{k}\right) = \mathcal{N}\left(\mathbf{w}_{k}|\mathbf{w}^{0}_{k}, \mathbf{A}_{k}\right),$ where $\mathbf{w}^{0}_{k}$ is a vector of size $n\times1$ and~$\mathbf{A}_{k}$ is a covariance matrix,
	\item[3)] the prior regularisation $p\left(\bm{\varepsilon}_{k,k'}|\bm{\Xi}\right) = \mathcal{N}\left(\bm{\varepsilon}_{k,k'}|\mathbf{0},  \bm{\Xi}\right),$ where~$\bm{\Xi}$ is a covariance matrix and $\bm{\varepsilon}_{k,k'} = \mathbf{w}_{k}^{0}-\mathbf{w}_{k'}^{0}.$
\end{enumerate}
The assumption 2) is a prior distribution for the parameters. It sets some restrictions to the model. For example, if~$\textbf{w}_k^{0}$ is [0,0,1], then the k-th local model prefers a circle with parameters $x_0=0, y_0=0, r=1$.

The assumption 3) is a prior regularisation. It sets restrictions on the prior distribution parameters. For example if~$\text{diag}\left(\bm{\Xi}\right)=[0.001, 0.001, 1]$, then the centers of different circles, which corresponds to different local models are equals.

Combining assumptions~$1), 2), 3)$ and equation~\eqref{eq:st:4}, obtain new likelihood function:
\[
\label{eq:em:1}
\begin{aligned}
p\bigr(\mathbf{y}, \mathbf{W}|\mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr) = &\prod_{i=1}^{N}\left(\sum_{k=1}^{K}\pi_{k}\mathcal{N}\left(y_{i}|\mathbf{w}_{k}^{\mathsf{T}}\mathbf{x}_{i}, \beta^{-1}\right)\right)\cdot\\
&\cdot\prod_{k=1}^{K}\mathcal{N}\left(\mathbf{w}_{k}|\mathbf{w}^{0}_{k}, \mathbf{A}_{k}\right)\prod_{k,k'=1}^{K}\mathcal{N}\left(\bm{\varepsilon}_{k,k'}|\mathbf{0},  \bm{\Xi}\right),
\end{aligned}
\]
...
Using the binary matrix~$\mathbf{Z}$ in~\eqref{eq:em:1} and take logarithm, obtain:
\[
\label{eq:em:2}
\begin{aligned}
...
\end{aligned}
\]
Set the new optimisation problem to optimise the evidence function, which is obtained by integrating formula~\eqref{eq:em:2} with respect to the parameters~$\textbf{W}, \textbf{Z}$:
\[
\label{eq:em:3}
\begin{aligned}
\mathbf{V}, \mathbf{W}^0, \textbf{A},  \beta = \arg\max_{\mathbf{V}, \mathbf{W}^0, \textbf{A}, \beta} \log p\bigr(\mathbf{y}|\mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr).
\end{aligned}
\]
\section{EM--algorithm as a solver of optimisation problem}
Let~$q\left(\textbf{W}, \textbf{Z}\right)$ be some density distribution of parameters~$\textbf{W}, \textbf{Z}$. Rewrite evidence:
\[
\label{eq:em:new:1}
\begin{aligned}
\log p\bigr(\mathbf{y}|\mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr) &= \int_{\textbf{W}, \textbf{Z}} q\left(\textbf{W}, \textbf{Z}\right) \log p\bigr(\mathbf{y}|\mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)d\textbf{W}d\textbf{Z} =\\
&= \int_{\textbf{W}, \textbf{Z}} q\left(\textbf{W}, \textbf{Z}\right)\log \frac{p\bigr(\mathbf{y}, \textbf{W}, \textbf{Z}|\mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)}{p\bigr(\textbf{W}, \textbf{Z}|\mathbf{y}, \mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)}d\textbf{W}d\textbf{Z}=\\
&= \int_{\textbf{W}, \textbf{Z}} q\left(\textbf{W}, \textbf{Z}\right)\log \frac{p\bigr(\mathbf{y}, \textbf{W}, \textbf{Z}|\mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)q\bigr(\textbf{W}, \textbf{Z}\bigr)}{p\bigr(\textbf{W}, \textbf{Z}|\mathbf{y}, \mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)q\bigr(\textbf{W}, \textbf{Z}\bigr)}d\textbf{W}d\textbf{Z}=\\
&= \int_{\textbf{W}, \textbf{Z}} q\left(\textbf{W}, \textbf{Z}\right)\frac{p\bigr(\mathbf{y}, \textbf{W}, \textbf{Z}|\mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)}{q\bigr(\textbf{W}, \textbf{Z}\bigr)}d\textbf{W}d\textbf{Z}+\\
&+\int_{\textbf{W}, \textbf{Z}} q\left(\textbf{W}, \textbf{Z}\right)\frac{q\bigr(\textbf{W}, \textbf{Z}\bigr)}{p\bigr(\textbf{W}, \textbf{Z}|\mathbf{y}, \mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)}d\textbf{W}d\textbf{Z}=\\
&=\mathcal{L}\left(q, \textbf{V}, \textbf{W}^{0}, \textbf{A}, \beta\right)+\mathsf{D}_{KL}\left(q\left(\textbf{W}, \textbf{Z}\right)||p\bigr(\textbf{W}, \textbf{Z}|\mathbf{y}, \mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)\right)
\end{aligned}
\]
...
\label{eq:em:new:2}
\begin{aligned}
\log p\bigr(\mathbf{y}|\mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)\geq \mathcal{L}\left(q, \textbf{V}, \textbf{W}^{0}, \textbf{A}, \beta\right),
\end{aligned}
\]
where~$\mathcal{L}\left(q, \textbf{V}, \textbf{W}^{0}, \textbf{A}, \beta\right)$ is called lower bound of evidence.

Use the expectation-maximization~\cite{bishop2006} algorithm to find the solution to optimisation problem~\eqref{eq:em:3}. The EM-algorithm instead of~$\log p\bigr(\mathbf{y}|\mathbf{X}, \mathbf{V}, \textbf{A}, \textbf{W}^{0}, \bm{\Xi}, \beta\bigr)$ optimizes the lower bound $\mathcal{L}\left(q, \textbf{V}, \textbf{W}^{0}, \textbf{A}, \beta\right)$.


\paragraph{E--step.} In the E-step, the EM-algorithm solves the following optimization problem:
\[
\label{eq:em:new:3}
\begin{aligned}
\mathcal{L}\left(q, \textbf{V}, \textbf{W}^{0}, \textbf{A}, \beta\right) \to \max_{q\left(\textbf{W}, \textbf{Z}\right)},
\end{aligned}
\]
where~$\textbf{V}, \textbf{W}^{0}, \textbf{A}, \beta$ are fixed.

Let a joint distribution~$q\left(\mathbf{Z}, \mathbf{W}\right)$ satisfies the assumption of independence~$q\left(\mathbf{Z}, \mathbf{W}\right) = q\left(\mathbf{Z}\right)q\left(\mathbf{W}\right)$~\cite{bishop2006}. 
...
\paragraph{M--step.}
In the M-step, the EM-algorithm solves the following optimization problem:
\[
\label{eq:em:new:3}
\begin{aligned}
\mathcal{L}\left(q, \textbf{V}, \textbf{W}^{0}, \textbf{A}, \beta\right) \to \max_{\textbf{V}, \textbf{W}^{0}, \textbf{A}, \beta},
\end{aligned}
\]
where~$q\left(\textbf{W}, \textbf{Z}\right)$ are fixed density function.
...
\end{aligned}
\]
First, to find the optimal parameters~$\textbf{V}$, use the gradient optimization algorithm. It convergences to some local maximum.
